...
function  addComments (url, request){
  const requestComment = request.body && request.body.comment;
  const response = {}
  if(requestComment && database.users[requestComment.username] && 
    database.articles[requestComment.articleId] && requestComment.body){
    const comment = {
...
function updateComments (url, request){
  ...
  if(comment && commentId){
    if(database.comments[commentId] && comment.body){
      database.comments[commentId].body = comment.body;
      response.body ={
        comment: comment
      };
      response.status = 200;
...
function deleteComment(url, request){
  ...
  if(commentId && database.comments[commentId]){
    ...
    for(const user in database.users){
      database.users[user].commentIds = database.users[user].commentIds.filter(currId => currId != commentId);
    }
    for(const article in database.articles){
      database.articles[article].commentIds = database.articles[article].commentIds.filter(currId => currId != commentId);
    }
    response.status = 204;
  } else {
    response.status = 404;
